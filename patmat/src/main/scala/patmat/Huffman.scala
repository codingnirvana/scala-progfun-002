...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
      def times(chars: List[Char], acc: List[(Char, Int)]) : List[(Char, Int)] = chars match {
        case List() => acc
        case x :: xs => times(xs, insert(x, acc))
      }

      def insert(c: Char, xs: List[(Char, Int)]) : List[(Char, Int)] = xs match {
        case List() => List((c, 1))
        case y :: ys => if (y._1 == c) (c, y._2 + 1) :: ys else y :: insert(c, ys)
      }
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    ...
    case left :: right :: xs => makeCodeTree(left, right) :: xs //insertCT(makeCodeTree(left, right),xs)
  }
   ...
   */
  def until[T](condition: T => Boolean , operate: T => T)(input: T): T = {
    val result = operate(input)
    if (condition(result)) result else until(condition, operate)(result)
  }
  ...
  def createCodeTree(chars: List[Char]): CodeTree = {
    val tree: List[Leaf] = makeOrderedLeafList(times(chars))
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def encode(t: CodeTree, c: Char): List[Bit] = t match {
